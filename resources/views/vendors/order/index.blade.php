          ...
          <div class="row">
            ...
            <div class="col-lg-2">
              <button class="btn btn-danger btn-sm d-none bulk-delete float-lg-right"
                data-href="{{ route('vendor.shop_management.bulk_delete_order') }}" class="card-header-button">
                  ...
                  <table class="table table-striped mt-2">
                    ...
                    <tbody>
                      @foreach ($orders as $order)
                        <tr>
                          ...
                          <td>
                            ...
                            @else
                              <form id="paymentStatusForm-{{ $order->id }}" class="d-inline-block"
                                action="{{ route('vendor.shop_management.order.update_payment_status', ['id' => $order->id]) }}"
                                method="post">
                                @csrf
                                <select
                          ...
                          <td>
                            <form id="orderStatusForm-{{ $order->id }}" class="d-inline-block"
                              action="{{ route('vendor.shop_management.order.update_order_status', ['id' => $order->id]) }}"
                              method="post">
                              @csrf
                              <select
                            ...
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                ...
                                <form class="deleteForm d-block"
                                  action="{{ route('vendor.shop_management.order.delete', ['id' => $order->id]) }}"
                                  method="post">
                                  @csrf
                        ...
                        @includeIf('vendors.order.show-receipt')
                      @endforeach
                    </tbody>
                  </table>
                </div>
