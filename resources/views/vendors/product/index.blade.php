        ...
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
              <a href="{{ route('vendor.shop_management.select_product_type') }}" class="btn btn-primary btn-sm float-right"><i class="fas fa-plus"></i> {{ __('Add Product') }}</a>

              <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete" data-href="{{ route('vendor.shop_management.bulk_delete_product') }}">
                    ...
                    <tbody>
                      @foreach ($products as $product)
                        <tr>
                          ...
                          @if ($themeInfo->theme_version == 2)
                            <td>
                              <form id="featuredForm-{{ $product->id }}" class="d-inline-block" action="{{ route('vendor.shop_management.product.update_featured_status', ['id' => $product->id]) }}" method="post">
                                @csrf
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm mr-1" href="{{ route('vendor.shop_management.edit_product', ['id' => $product->id, 'type' => $product->product_type]) }}">
                              <span class="btn-label">
                                <i class="fas fa-edit"></i>
                              </span>
                            </a>

                            <form class="deleteForm d-inline-block" action="{{ route('vendor.shop_management.delete_product', ['id' => $product->id]) }}" method="post">
                              @csrf
